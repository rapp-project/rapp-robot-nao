...
if [ "$#" -ne 1 ]; then
	echo "$COL_RED[Error]$COL_RESET - Usage: $0 <flag>"
...
if [ -d $GIT_WS_RAPP_APPLICATIONS_DIR ]; then #If directory exists
	if [ $1 -eq 1 ]; then #clone from github
		echo "$COL_GREEN[OK]$COL_RESET - Removing $GIT_WS_RAPP_APPLICATION_DIR - a directory. Cleaning git folder."
...
if [ -d $WS_RAPP_APPLICATIONS_NAO_DIR ]; then #If directory exists
	echo "$COL_GREEN[OK]$COL_RESET - Directory $WS_RAPP_APPLICATIONS_NAO_DIR exists"
	if [ $1 -eq 1 ]; then #clone from github
		echo "$COL_GREEN[OK]$COL_RESET - Removing $WS_RAPP_APPLICATIONS_NAO_DIR - a directory. Cleaning git folder."
		rm $WS_RAPP_APPLICATIONS_NAO_DIR -rf
		mkdir -p $WS_RAPP_APPLICATIONS_NAO_DIR
	fi
else
	echo "$COL_GREEN[OK]$COL_RESET - Creating - a directory. Creating $WS_RAPP_APPLICATIONS_NAO_DIR directory."
	mkdir -p $WS_RAPP_APPLICATIONS_NAO_DIR
fi

if [ $1 -eq 1 ]; then #clone from github
	cd $GIT_WS_RAPP_NAO_DIR
	echo "$COL_GREEN[OK]$COL_RESET - Clonning rapp-robot-nao repository to $GIT_WS_RAPP_NAO_DIR"
	...
	echo "$COL_GREEN[OK]$COL_RESET - Clonning rapp-robot-nao repository to $GIT_WS_RAPP_APPLICATIONS_DIR"
	echo "Enter your github login and password"
	git clone -b master https://github.com/rapp-project/rapp-applications.git
fi

echo "$COL_GREEN[OK]$COL_RESET - Copying dynamic agent packages to $WS_RAPP_APPLICATIONS_NAO_DIR directory"
cp -r $GIT_WS_RAPP_APPLICATIONS_DIR/rapp-applications/nao/src $WS_RAPP_APPLICATIONS_NAO_DIR

echo "$COL_GREEN[OK]$COL_RESET - Creating folder $HZ_DIRECTORY"
...
if [ -d $ROS_ADDITIONAL_PACKAGES_SRC_DIR ]; then #If directory exists
	...
	echo "$COL_GREEN[OK]$COL_RESET - Creates $ROS_ADDITIONAL_PACKAGES_SRC_DIR"
	mkdir -p $ROS_ADDITIONAL_PACKAGES_SRC_DIR
fi

echo "$COL_GREEN[OK]$COL_RESET - Updates virtual machine scripts in $VM_SCRIPTS folder"
cd $VM_SCRIPTS
rm vm*
