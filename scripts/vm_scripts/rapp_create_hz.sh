...
RAPP_PATH="/home/nao"
RAPP_DIRECTORY=$RAPP_PATH"/rapp/hz_packages/$1"
...
if [ -d $RAPP_DIRECTORY ]; then #If package exists
  ...
  rm $RAPP_PATH/rapp/hz_packages/$1 -rf
fi

mkdir -p $RAPP_PATH/rapp/hz_packages/$1/rapp_dynamic_agent/lib
mkdir -p $RAPP_PATH/rapp/hz_packages/$1/rapp_dynamic_agent/share

echo "Copying - $1 libs and shares folders to corresponding folders in: $RAPP_DIRECTORY"
cp -r $RAPP_WORKSPACE/install_isolated/lib/$1 $RAPP_PATH/rapp/hz_packages/$1/rapp_dynamic_agent/lib/
cp -r $RAPP_WORKSPACE/install_isolated/share/$1 $RAPP_PATH/rapp/hz_packages/$1/rapp_dynamic_agent/share/

echo "Copying - hz template files to temporary catalog for package: $1"
cp -r $RAPP_PATH/rapp/hz_template/template_package/* $RAPP_PATH/rapp/hz_packages/$1/

echo "Creates - hz package $1-1.0.0.hz"
tar czvf $RAPP_PATH/rapp/hz_packages/$1-1.0.0.hz $RAPP_PATH/rapp/hz_packages/$1
