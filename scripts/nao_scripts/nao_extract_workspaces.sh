...
echo "$COL_GREEN[OK]$COL_RESET - Creates workspaces"
mkdir -p $WS_ROS
mkdir -p $WS_ROS_ADDITIONAL_PACKAGES
mkdir -p $WS_RAPP_NAO
mkdir -p $WS_RAPP_APPLICATIONS_NAO_INSTALL_ISOLATED

echo "$COL_GREEN[OK]$COL_RESET - Extracts archived files into correct workspaces"
echo "$COL_GREEN[OK]$COL_RESET - $WS_ROS"
cd $WS_ROS
tar zxf /home/nao/ws_ros.tar.gz

echo "$COL_GREEN[OK]$COL_RESET - $WS_ROS_ADDITIONAL_PACKAGES"
cd $WS_ROS_ADDITIONAL_PACKAGES
tar zxf /home/nao/ws_ros_additional_packages.tar.gz

echo "$COL_GREEN[OK]$COL_RESET - $WS_RAPP_NAO"
cd $WS_RAPP_NAO
tar zxf /home/nao/ws_rapp_nao.tar.gz

echo "$COL_GREEN[OK]$COL_RESET - Extracts nao data"
cd $WS_RAPP_APPLICATIONS_NAO
tar zxf /home/nao/nao_data.tar.gz

echo "$COL_GREEN[OK]$COL_RESET - Removes archived files"
rm /home/nao/ws_ros.tar.gz /home/nao/ws_ros_additional_packages.tar.gz /home/nao/ws_rapp_nao.tar.gz

echo "$COL_GREEN[OK]$COL_RESET - Removes nao_scripts.tar.gz"
