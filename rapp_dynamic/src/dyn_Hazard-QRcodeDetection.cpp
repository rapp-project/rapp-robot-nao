	...
	while (check==true)
{
	...
	else //camera frame is not empty
	{
		...
		for (int i=0; i<QRcodeDetectionStruct.QRmessage.size(); i++) //QRcode detected
		{
			...
			//NAO_Vision.textToSpeech( QRcodeDetectionStruct.QRmessage[i] );//reads the QRcode message
		//	NAO_Vision.textToSpeech("Q R code detected ");
		//	NAO_Vision.textToSpeech( text_message );//reads the QRcode message
		...
		if (QRcodeHazardStruct.isHazardFound == true) //Open door detected
		{
			...
			for (int i=0; i<QRcodeHazardStruct.openedObject.size(); i++)
			{
				...
				//NAO_Vision.textToSpeech( QRcodeHazardStruct.openedObject[i] + std::string(" is open") );
				NAO_Vision.textToSpeech( text_message + std::string(" is open") );
